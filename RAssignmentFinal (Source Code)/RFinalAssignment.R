...
display1 <- ggplot(report %>%
  ...
  labs(title = ' Temperature VS Dew point Boxplot',
       x = 'Temperature ( ÂºF )', y = 'Dewpoint ( ÂºF )')
print(display1)
...
display3<- ggplot(report, mapping = aes(x =month, y = temp,
                                        color = origin )) +
  ...
  labs(title = 'month Vs Temperature ',
...
display4<-hist(report$temp,
               main = "Frequency of Temperature Histogram",
               xlab = "Temperature ( AºF )",ylab = "Frequency ( No.)",
               las = 1,
               col = c("red", "black"))
print(display4)
...
display5<-ggplot(data = report, mapping = aes(x = visib, na.rm = TRUE)) +
...
display6<-plot(report$month,report$pressure, col = "pink", pch = 2, 
...
##visualization 
display7<- ggplot(report, mapping = aes(x = wind_speed, 
...
display8<- ggplot(report, mapping = aes(x = hour, y = wind_dir,
...
display9<- ggplot(report, mapping = aes(x =wind_dir)) + 
...
display10<-ggplot(report,aes(x = hour,y = temp)) + 
...
display11<-ggplot(report, mapping = aes(x = humid)) + 
  geom_histogram(col="green",fill="burlywood1") + labs(title = 'Histogram of Humidity',
                                                       x = 'Humidity')
print(display11+day_plot)



#Analysis Example_12 : -
##In this example, At the origin, an analysis is performed between x and y for month vs humidity.
#visualization and Exploration

display12 <- ggplot(report) +
